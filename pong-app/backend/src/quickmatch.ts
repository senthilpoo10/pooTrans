...
import { playersOnline, pongRooms, keyClashRooms, getQuickMatchState } from "./gameData";
import PingPongGame from "./PingPongGame";
import { state } from "./KeyClashGame";

export function setupQuickMatch(io: Server) {
    const quickmatchNamespace = io.of('/quickmatch');

    quickmatchNamespace.on("connection", (socket: Socket) => {
      ...
      socket.on("name", (name: string | null, playerId: string | null, callback: Function) => {
        ...
        quickmatchNamespace.emit("lobby_update", getQuickMatchState());
      })
