...
export default function lobbyRoutes(fastify: FastifyInstance, options: LobbyRoutesOptions) {
  ...
  fastify.post('/profile/update', async (request: UpdateProfileRequest, reply: FastifyReply) => {
    try {
  ...
  // POST /lobby/friend-requests/send - For RallySquadTab friend requests
  fastify.post<{ Body: { receiverId: number } }>('/friend-requests/send', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
  ...
  // GET /lobby/friend-requests - For RallySquadTab incoming requests
  fastify.get('/friend-requests', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
  ...
  // POST /lobby/friend-requests/:requestId/respond - For RallySquadTab request response
  fastify.post<{ 
    Params: { requestId: string }, 
    Body: { action: 'accept' | 'decline' } 
  }>('/friend-requests/:requestId/respond', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
      ...
      if (action === 'accept') {
        // Update both friendship records to 'Friend' status
  ...
  // DELETE /lobby/friendships/:friendshipId - For RallySquadTab remove friend
  fastify.delete<{ Params: { friendshipId: string } }>('/friendships/:friendshipId', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
  ...
  // GET /lobby/friends/enhanced - For RallySquadTab with friendshipId
  fastify.get('/friends/enhanced', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
