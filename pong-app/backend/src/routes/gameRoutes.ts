...
export default function gameRoutes(fastify: FastifyInstance, options: { prisma: PrismaClient }) {
  ...
  fastify.post<{ Body: GameSaveRequest }>('/games/save', async (request, reply) => {
    try {
      ...
      const userPlayer = userIsPlayer1 ? player1Data : player2Data;
      const opponentPlayer = userIsPlayer1 ? player2Data : player1Data;
      ...
      if (userPlayer.username === user.username) {
      ...
      // For remote games, try to update opponent stats (if they exist and it's a registered user)
      if (mode === 'remote') {
        try {
          const opponent = await prisma.user.findUnique({
            where: { username: opponentPlayer.username }
          });
          
          if (opponent) {
            if (opponentPlayer.isWinner) {
              await prisma.user.update({
