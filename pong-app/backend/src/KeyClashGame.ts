...
import { keyClashRooms, getQuickMatchState, keyClashTournaments, getTournamentState } from "./gameData.js";
...
export function setupKeyClash(io: Server) {
    ...
    const quickmatch = io.of("/quickmatch");
    ...
    keyClash.on("connection", (socket) => {
        ...
        socket.on("join_game_room", (roomId, mode, type, callback) => {
            ...
            socket.on("names", (names) => {
                ...
                if (type === "1v1")
                    quickmatch.emit("lobby_update", getQuickMatchState());
                else
                    tournament.emit("lobby_update", getTournamentState());
