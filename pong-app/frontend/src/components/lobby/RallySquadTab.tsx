...
import { 
  ...
  type RallySquadData,
  type User 
} from '../../utils/lobbyApi'; // Use lobbyApi instead of friendApi
...
export const RallySquadTab = () => {
  ...
  const handleSendFriendRequest = async (userId: string) => {
    try {
      console.log('Sending friend request to user ID:', userId); // Debug log
      await sendFriendRequest(userId);
      setSuccessMessage("Friend request sent!");
      setTimeout(() => setSuccessMessage(""), 3000);
      ...
      setTimeout(loadRallySquadData, 1000);
    } catch (err: any) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setSentRequests([]); // Clear sent requests state
  ...
  const getUserFriendStatus = (user: User) => {
    ...
    // Check if we sent a request to this user
    if (sentRequests.includes(user.id.toString())) {
      return {
        status: 'Request Sent',
        type: 'pending',
        bgColor: 'bg-yellow-500'
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-4 text-sm text-gray-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>Friend Requests: {rallySquadData?.friendRequests.length || 0}</div>
        </div>
