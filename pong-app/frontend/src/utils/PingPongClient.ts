...
export default class PingPongClient {
  ...
  constructor(containerId: string | HTMLElement, gameId: string, mode: "local" | "remote", type: "1v1" | "tournament", navigate: NavigateFunction, names: { player1: string | null, player2: string | null, player3: string | null, player4: string | null } | string | null) {
    if (typeof containerId === 'string') {
      const el = document.getElementById(containerId);
      if (!el) throw new Error(`Container with id "${containerId}" not found`);
      this.container = el as HTMLDivElement;
  ...
  private connect(){
    ...
    this.socket.on('gameOver', (state) => {
      // Save game result
      const winner = state.player1.score > state.player2.score ? 
                    state.player1.name : state.player2.name;
      const loser = state.player1.score > state.player2.score ? 
                   state.player2.name : state.player1.name;
      const finalScore = `${state.player1.score} - ${state.player2.score}`;
      
      this.saveGameResult(winner, loser, finalScore, state.duration || 0);
    });
