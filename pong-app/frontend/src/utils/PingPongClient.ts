...
export default class PingPongClient {
  ...
  private currentLeftScore: number = 0;
  private currentRightScore: number = 0;
  private leftPlayerName: string = '';
  private rightPlayerName: string = '';
  private gameRounds: any[] = [];
  ...
  constructor(
    ...
    if (gameState) {
      this.userAvatar = gameState.userAvatar?.name || 'default';
      this.guestAvatar = gameState.guestAvatar?.name || 'default';
  ...
  private connect(name: string | null){
    ...
    this.socket.on('stateUpdate', (state, start: string | null) => {
        ...
        if (this.scoreDisplay.textContent !== state.scoreDisplay) {
          ...
          // Parse and track scores for saving
          this.parseScoreDisplay(state.scoreDisplay);
        }
        ...
        if (state.status === "finished" && this.gameStartTime > 0) {
  ...
  private parseScoreDisplay(scoreDisplay: string) {
    // Parse score display like "Player1: 2  —  Player2: 1"
    const regex = /(.+):\s*(\d+)\s*—\s*(.+):\s*(\d+)/;
    const match = scoreDisplay.match(regex);
    
    if (match) {
      this.leftPlayerName = match[1].trim();
      this.currentLeftScore = parseInt(match[2]);
      this.rightPlayerName = match[3].trim();
      this.currentRightScore = parseInt(match[4]);
    }
  }

  private async handleGameFinished(state: any) {
    try {
      // Only save for the main user (not for tournaments where this might be called multiple times)
      if (this.type === "tournament") {
        return; // Skip saving for tournament games for now
      }

      const gameDuration = Math.floor((performance.now() - this.gameStartTime) / 1000);
      
      // Determine winner from final state
      const winner = state.matches && state.matches.length > 0 ? state.matches[state.round - 1]?.winner : null;
      let leftPlayerWon = false;
      let rightPlayerWon = false;

      if (winner) {
        leftPlayerWon = winner.name === this.leftPlayerName || winner.side === "left";
        rightPlayerWon = !leftPlayerWon;
      } else {
        // Fallback to score comparison
        leftPlayerWon = this.currentLeftScore > this.currentRightScore;
        rightPlayerWon = this.currentRightScore > this.currentLeftScore;
      }
      ...
      console.log('Saving pong game result:', gameResult);
      ...
      console.error('Failed to save pong game result:', error);
      this.showGameSaveError();
    }
  }

  private showGameSavedMessage() {
